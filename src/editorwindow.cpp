...
EditorWindow::EditorWindow(QApplication* app)
{
    ...
    filters["Map Source Files (*.h & *.c)"] =
        std::make_shared<SaveSourcesCommand>();
    filters["Serialized map data (*.json)"] =
        std::make_shared<SaveJsonCommand>();
}
...
void EditorWindow::setup_triggers(Ui_MainWindow* ui)
{
    ...
    QObject::connect(ui->actionSave_As, SIGNAL(triggered()), this,
                     SLOT(on_save_as()));
    QObject::connect(ui->actionQuit, SIGNAL(triggered()), this,
                     SLOT(on_quit()));
    QObject::connect(ui->actionUndo, SIGNAL(triggered()), this,
                     SLOT(on_undo()));
    QObject::connect(ui->actionRedo, SIGNAL(triggered()), this,
                     SLOT(on_redo()));
    QObject::connect(ui->actionZoom_In, SIGNAL(triggered()), this,
                     SLOT(on_zoom_in()));
    QObject::connect(ui->actionZoom_Out, SIGNAL(triggered()), this,
                     SLOT(on_zoom_out()));
    QObject::connect(ui->actionChange_Properties, SIGNAL(triggered()), this,
                     SLOT(on_change_properties()));
    QObject::connect(ui->actionShow_Grid, SIGNAL(triggered()), this,
                     SLOT(on_grid()));
...
void EditorWindow::on_new()
{
    ...
    if (choice != -1)
    {
        ...
        if (dialog->is_regular())
        {
            ...
            /* REGULAR maps */
            case 0:
                map = new Map(32, 32, true);
                break;
            case 1:
                map = new Map(32, 64, true);
                break;
            case 2:
                map = new Map(64, 32, true);
                break;
            case 3:
        ...
        else
        {
            ...
            /* AFFINE maps */
            case 0:
                map = new Map(16, 16, false);
                break;
            case 1:
                map = new Map(32, 32, false);
                break;
            case 2:
                map = new Map(64, 64, false);
                break;
            case 3:
                map = new Map(128, 128, false);
                break;
            }
        }
...
void EditorWindow::on_open()
{
    ...
    filename = QFileDialog::getOpenFileName(
        this, tr("Open File"), "",
        tr("Serialized map data (*.json);;All files (*)"));
    if (filename != "")
    {
        filename_valid = true;
        if (map)
...
QString EditorWindow::get_save_name()
{
    QString f = QFileDialog::getSaveFileName(
        this, tr("Save File"), "",
        tr("Map Headers (*.h);;Map Source Files (*.h & *.c);;Serialized map "
           "data (*.json)"),
        &chosen_save_filter);
    return f;
}
...
void EditorWindow::save_to_file()
{
    if (map)
    {
        if (!chosen_save_filter.isNull()
            && !chosen_save_filter.trimmed().isEmpty())
        {
...
void EditorWindow::on_change_properties()
{
    ...
    QString file = QFileDialog::getOpenFileName(this, tr("Set Image"), "",
                                                tr("Image Files (*.png)"));
...
void EditorWindow::updateTilePreviewIcon()
{
    ...
    activeTileInToolbarAction->setIcon(
